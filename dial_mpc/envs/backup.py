...
class UnitreeAliengoEnv(BaseEnv):
    def __init__(self, config: UnitreeAliengoEnvConfig):
        ...
        self.joint_torque_range = jnp.array(
            [
                [-35, 35],
                [-35, 35],
                [-45, 45],
    ...
    def step(self, state: State, action: jax.Array) -> State:
        ...
        def exp_shaping(x, norm=1.0, scale=1.0):
            # Funzione esponenziale che penalizza i valori più alti.
            return jnp.exp(-x / norm) * scale  # Penalizza valori più alti
        ...
        reward_gaits = exp_shaping(jnp.sum(((z_feet_tar - z_feet) / 0.05) ** 2), norm=0.25, scale=2.0)
        # reward_gaits = 1/jnp.sum(((z_feet_tar - z_feet) / 0.05) ** 2)
        ...
        reward_air_time = exp_shaping(jnp.sum((state.info["feet_air_time"] - 0.1) * first_contact), norm=0.25, scale=1.0)
        ...
        reward_pos = exp_shaping(jnp.sum((head_pos - pos_tar) ** 2), norm=0.25, scale=1.0)

        # stay upright reward
        vec_tar = jnp.array([0.0, 0.0, 1.0])
        vec = math.rotate(vec_tar, x.rot[0])
        reward_upright = exp_shaping(jnp.sum(jnp.square(vec - vec_tar)), norm=1.0, scale=1.0)
        ...
        d_yaw = yaw - yaw_tar
        reward_yaw = exp_shaping(jnp.square(jnp.atan2(jnp.sin(d_yaw), jnp.cos(d_yaw))), norm=0.25, scale=0.5)
        ...
        reward_vel = exp_shaping(jnp.sum((vb[:2] - state.info["vel_tar"][:2]) ** 2), norm=0.25, scale=1.0)
        reward_ang_vel = exp_shaping(jnp.sum((ab[2] - state.info["ang_vel_tar"][2]) ** 2), norm=0.25, scale=0.5)
        ...
        first_contact = (state.info["feet_air_time"] > 0) * contact_filt_mm
        state.info["feet_air_time"] += self.dt
        reward_air_time = jnp.sum((state.info["feet_air_time"] - 0.2) * first_contact)
        # position reward
        pos_tar = (
            state.info["pos_tar"] + state.info["vel_tar"] * self.dt * state.info["step"]
        )
        ...
        cstr_torque = (jnp.abs(ctrl) - torque_limit) / torque_limit
        ...
        cstr_air_time = (0.2 - state.info["feet_air_time"]) / 0.2 * first_contact
        ...
        # Foot height threshold when foot is not in contact
        height_threshold = 0.05
        cstr_foot_height = 0.0 * jnp.where(foot_contact_z >= 1e-3, 0.0, (height_threshold - z_feet) / height_threshold)
